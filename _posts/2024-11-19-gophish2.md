...
Gmail은 따로 설정이 없다. 단 IMAP이나 POP3를 사용하려면 설정을 해야 하지만 여기는 없다.
...
- SMTP From은 속이는 계정으로 메일을 받았을 때 여기에 적은 계정 이름으로 메일이 온다.
...
<img src="/assets/images/gophish/05.naver_main.png"/>

네이버 로그은 후 프로필에서 네모칸 안에 있는 버튼인 설정버튼을누르면

<img src="/assets/images/gophish/06.naver_secset.png"/>

좌측에 보안설정으로 들어가서 2단계인증을 진행 한다.

2단계 인증이 완료되면 2단계 인증이 설정->관리로 변경 되는데 관리로 들어가 보면

<img src="/assets/images/gophish/07.naver_set_apppassword.png"/>

애플리케이견 비밀번호 관리 항목이 있다. 여기서 애플리케이션 비밀번호를 생성하면 "3.비밀번호 확인" 항목에 비밀번호가 뜨는데 이것을 Password에 입력하면 된다.
...
피싱 사이트를 작성하는 곳으로 여기서 아이디 및 비밀번호 탈취, 악성코드 설치 등 가능하지만 여기서는 
